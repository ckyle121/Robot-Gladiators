...
// initialize computer enemy Robots
var enemyInfo = [
  {
    name: "Roborto",
...
var startGame = function(){
    ...
    for (var i = 0; i < enemyInfo.length; i++) {
        if (playerInfo.health > 0){
            ...
            // pass pickedenemy.name variable value into fight function
            fight(pickedEnemyObj);
            
            // if player is still alive AND we're not the last enemy in the array 
            if (playerInfo.health > 0 && i < enemyInfo.length - 1){
                // ask play if they want to go to store before next round
                var storeConfirm = window.confirm("The fight is over, visit the store before the next round?");
                
                // if player confirms, take them to store function
                if (storeConfirm){
                    shop();
                }
            }
...
var endGame = function(){
  ...
  //if player has more moeny than the high score, player has new high score
  ...
  if (playAgainConfirm){
    ...
    window.alert("Thank you for playing Robot Gladiators! Come back and battle again soon.");
  }
